...
Expected: a table starting `QUALIFYING FONTS (10)` including D2Coding, M PLUS 1 Code, GNU Unifont, UnifontEX, and the six Fairfax variants (first run downloads ~20MB into `.cache`).
...
- `bun run fonts:list --json | bun -e "const j=await Bun.stdin.json(); console.log(j.filter(x=>x.qualifies).map(x=>x.name).join('\n'))"` — prints the 10 known qualifying fonts (D2Coding, M PLUS 1 Code, GNU Unifont, UnifontEX, Fairfax ×6).
- `bun run dev` then `curl localhost:3000/api/fonts` — JSON array of the 10 qualifying fonts; `curl localhost:3000/` — HTML containing 「中英倍宽字体」.
